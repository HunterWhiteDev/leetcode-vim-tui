...
void solveQuestion(char *tokenHeaderStr, char *fileName, char *csrfToken) {
  ...
  int problemUrlLen =
      snprintf(NULL, 0, "https://leetcode.com/problems/%s/submit/", slug);
  char *problemUrl = malloc(problemUrlLen + 1);
  snprintf(problemUrl, problemUrlLen + 1,
           "https://leetcode.com/problems/%s/submit/", slug);
  ...
  if (curl) {
    ...
    if (result != CURLE_OK)
      ...
      char *interpretId =
          cJSON_Print(cJSON_GetObjectItem(responseJson, "submission_id"));
